...
$$
y = \sum_{i = 1}^{n} G \left(\right. x \left.\right)_{i} E_{i} \left(x\right)
$$
...
$$
G_{\sigma} \left(x\right) = \text{Softmax} \left(x \cdot W_{g}\right)
$$
...
$$
H \left(\right. x \left.\right)_{i} = \left(\right. x \cdot W_{\text{g}} \left.\right)_{i} + \text{StandardNormal}() \cdot \text{Softplus} \left(\right. \left(x \cdot W_{\text{noise}} \left.\right)_{i}\right)
$$

2. We only pick the top k

$$
\text{KeepTopK} \left(\right. v , k \left.\right)_{i} = \begin{cases} v_{i} & \text{if } v_{i} \text{is in the top } k \text{elements of } v , \\ - \infty & \text{otherwise}. \end{cases}
$$

3. We apply the softmax.

$$
G \left(x\right) = \text{Softmax} \left(\text{KeepTopK} \left(H \left(x\right) , k\right)\right)
$$
...
![](https://huggingface.co/blog/assets/intel-qwen3-agent/smolagents-1300-650.png)
...
September 29, 2025 ![](https://huggingface.co/blog/assets/cosmopedia/thumbnail.png)
