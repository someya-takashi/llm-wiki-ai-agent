...
source: "https://medium.com/@tungvu_37498/understanding-llm-serving-how-to-run-language-models-fast-cheap-and-effectively-70ef68242d93"
author:
  - "[[Thanh Tung Vu]]"
...
![](https://miro.medium.com/v2/resize:fit:1358/format:webp/0*ZN00MeZSpfxQWdcS.png)
...
There are several methods for adapting LLMs to downstream tasks (check my previous [post](https://medium.com/@tungvu_37498/adapting-a-pre-trained-llm-to-a-specific-task-domain-or-behavior-1cf46314c2a7)):
...
- **LoRA (Low-Rank Adaptation)**: Inject trainable low-rank matrices into specific weight components (e.g., attention layers). Ideal for adapting large models with minimal additional memory.
- **Adapter modules**: Insert lightweight bottleneck layers between transformer blocks. Commonly used in multi-task learning and multilingual settings.
...
![](https://miro.medium.com/v2/resize:fit:1176/format:webp/1*wIFwOcK4Bq3MRrpJfvsfsQ.png)
...
![](https://miro.medium.com/v2/resize:fit:1400/format:webp/1*2e1-1mIqR5Jm75kUbPAKtQ.png)
...
- **Example**: A system that detects a user is asking about legal policy and routes the request to a law-specific model instead of a general-purpose LLM.
...
![](https://miro.medium.com/v2/resize:fit:1400/format:webp/1*onDLRq-RWfBCp9IeluaLnw.png)
...
**GPTQ (Gradient Post-Training Quantization)**: Post-training quantization method that uses second-order optimization to minimize the loss introduced by quantization.
...
![](https://miro.medium.com/v2/resize:fit:1400/format:webp/1*F_ubVf48tdt7FztkfAfb4Q.png)
...
- `Triton`: NVIDIA’s inference server supports multiple backends (PyTorch, TensorFlow, ONNX, etc.). Offers dynamic batching, model versioning, and GPU-accelerated token streaming.
...
**Cloud Platforms**: AWS (SageMaker, Bedrock), Azure (Azure ML, OpenAI Service), and GCP (Vertex AI) provide managed infrastructure for deploying LLMs at scale. These services are ideal for enterprises looking for integration, compliance, and scale without managing the serving infrastructure manually.
