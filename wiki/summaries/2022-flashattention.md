...
5. **「最適」の範囲は狭く取るべき**。定数倍を除いた漸近的最適であって、実装レベルではまだ改善余地があった。実際 FlashAttention-2（2023）は同じ IO 計算量のまま work partitioning を直して約 2 倍速くなっている。**同じ漸近計算量の中に 2 倍が残っていた**という事実自体、「FLOPs もオーダーも wall-clock の代理指標としては粗い」という本論文の教訓の再演になっている。
