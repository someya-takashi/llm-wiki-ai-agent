...
本 wiki は Moonshot / DeepSeek のモデルレポートを系譜として追ってきた——**K2（[[summaries/2025-kimi-k2]]）→ K2.5（[[summaries/2026-kimi-k2.5]]）→ DeepSeek-V4（[[summaries/2026-deepseek-v4]]）→ gpt2-to-kimi3（[[summaries/2026-gpt2-to-kimi3]]）**。K3 はその直系の続きで、アーキテクチャの土台（KDA・AttnRes・Stable LatentMoE）は gpt2-to-kimi3 が既に概説している。**KDA とハイブリッド・NoPE の直接の前身は Kimi Linear（[[summaries/2025-kimi-linear]]）で、K3 の KDA はこれを下限つき減衰・全ランクゲートで精緻化したもの。**
...
- **参照原典の多くが未取得。** AttnRes [^59]・LatentMoE [^31]・MoonViT-V2・EAGLE-3 [^70]・MXFP4 [^102] は本 wiki に未取得。**KDA/Kimi Linear [^56] は 2026-08-11 に取得済み（[[summaries/2025-kimi-linear]]）で、KDA・DPLR・NoPE・効率の記述はこれで一次資料に接地した**（残る未取得分はアーキテクチャの記述に又聞きを含む）。
...
- [[summaries/2025-kimi-linear]] — KDA・ハイブリッド・NoPE の直接の前身（K3 はこれを精緻化）
