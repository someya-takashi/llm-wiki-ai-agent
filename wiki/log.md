...
## [2026-08-04] ingest | Code as Agent Harness: Toward Executable, Verifiable, and Stateful Agent Systems

- 取り込み: `raw/papers/Code as Agent Harness ◆ Toward Executable, Verifiable, and Stateful Agent Systems ◆.md`（Xuying Ning ほか全 42 名 / UIUC・Meta・Stanford, arXiv:2605.18747, 2026。ケース A: ar5iv クリップ）
- 作成: [[translations/2026-code-as-agent-harness]], [[summaries/2026-code-as-agent-harness]], `raw/assets/2026-code-as-agent-harness/x1.png`〜`x12.png`（12 枚）
- 更新: [[concepts/harness-engineering]], [[concepts/multi-agent-systems]], [[concepts/agent-evaluation]], [[concepts/agent-safety-and-guardrails]], [[overview]], [[index]]
- クリップの状態: **画像 12 枚・図表キャプション 23 件（Figure 1〜12・Table 1〜11）・表 11 個・SVG のテキスト箱 2 個はすべて残存**していた。**欠落は 1 件のみ**——ar5iv の見出し 143 に対しクリップは 142 で、差分は §4.4 の小見出し **"Context management is the tax of implicit shared state"** が段落ごと丸ごと。原ページから復元して §4.4 の該当位置（トポロジ複雑さの節と役割特化の節の間）へ挿入した。この論文は**脚注を持たない**（本文中の `<sup>N</sup>` は著者の所属記号）。ar5iv 側にある大学ロゴ 3 枚は所属表示の画像なのでサイト装飾として除外。タイトルの `◆` は原典の LaTeX `\lozenge` 由来の装飾で、クリップに `◆\\lozenge` と漏れているのはレンダリングの副産物。引用が裸の bibkey なのは ar5iv 側に文献一覧が生成されていないためでクリップ不良ではない（既存の慣例に従い bibkey を維持）。
- 翻訳の範囲: 本文 §1〜5 の**全 69 の番号つき見出しを全訳**。**表 11 個も全訳**（サーベイ翻訳の前例に従い、列見出しと分析列を日本語化し、手法名・システム名・bibkey は原文のまま）。参考文献一覧と謝辞は既定どおり除外。原典に §6（結論）は存在しない。
- ユーザーの決定（AskUserQuestion）: **(1) 概念ページは 4 ページに絞る**——`harness-engineering`（3 要素の区別・PEV ループ・change contract の一般化・蒸留面としてのハーネス）／`multi-agent-systems`（共有ハーネス基盤の 4 水準と The Central Gap・収束の 6 パターン・トポロジ複雑さの逆相関・文脈管理は暗黙状態の税）／`agent-evaluation`（ハーネス水準の指標 6 次元・oracle adequacy・evidence bundle・模擬実行 98% の留保）／`agent-safety-and-guardrails`（耐久的なハーネス状態としての HITL・executable accountability・権限の多層モデルと文脈依存性）。**(2) 応用領域は要約ページに留める**——身体化エージェント・科学的発見・パーソナライゼーションは概念ページを新設せず、要約ページ内で扱う。
- **データギャップ（上記 (2) の理由）**: 本サーベイが依拠する主要原典のうち、**身体化エージェント**（Voyager / SayCan / Code as Policies）と**科学的発見**（ChemCrow / Coscientist / Virtual Lab / Biomni / AlphaEvolve / AlphaProof）は本 wiki に 1 件も無く、[[summaries/2026-ai-scientist]] が科学領域の唯一の足がかり。またサーベイが 2026 年のプレプリントとして引く AutoHarness / Natural-Language Agent Harnesses / SyncMind / L2MAC / EvoMAC / QualityFlow も未取得で、これらの記述は本サーベイ経由の又聞きに留まる。**原典が入った時点で `embodied-agents` 系の概念ページ新設を検討する**（`research-agents` は既存）。パーソナライゼーション（推薦系のエージェント化）も同様に原典ゼロ。
- メモ: 本サーベイの最大の収穫は **PEV ループ**と **The Central Gap** の 2 つ。前者は [[concepts/harness-engineering]] の設計論と [[concepts/agent-safety-and-guardrails]] の権限設計を 1 本のループの中で接続し、後者は [[summaries/2025-masft]] の失敗分類（症状）に対する**原因側**の記述を与える——MASFT の FC1/FC2（会話履歴の喪失・情報の抱え込み）も FC3（早すぎる終了・検証の欠落）も、「形式的な共有基盤がない」の帰結として読み直せる。もう 1 つ記録に値するのは、サーベイが**自らの中心的な売りを掘り崩す証拠を併記している**こと——QualityFlow の Imagined Execution が MBPP で適合率・再現率 98% 超を「コードを走らせずに」達成しており、「実行できるから客観的なオラクルが得られる」という主張の適用範囲をコーナーケースへ絞り込んでいる。

## [2026-08-04] ingest | Externalization in LLM Agents: A Unified Review of Memory, Skills, Protocols and Harness Engineering

- 取り込み: `raw/papers/Externalization in LLM Agents_ A Unified Review of Memory, Skills, Protocols and Harness Engineering.md`（Chenyu Zhou ほか全 21 名 / 上海交通大学・中山大学・上海创智学院・CMU・OPPO, arXiv:2604.08224, 2026-05-05。ケース A: ar5iv クリップ）
- 作成: [[translations/2026-externalization]], [[summaries/2026-externalization]], **[[concepts/agent-skills]]（新設）**, `raw/assets/2026-externalization/`（x1〜x3・timeline・circle・memory_f・skill_f・protocol_f の 8 枚）
- 更新: [[concepts/harness-engineering]], [[concepts/agent-memory]], [[concepts/model-context-protocol]], [[concepts/context-engineering]], [[concepts/tool-use-and-function-calling]], [[concepts/agent-evaluation]], [[concepts/agent-safety-and-guardrails]], [[overview]], [[index]]
- クリップの状態: **画像 8 枚・図キャプション 8 件（Figure 1〜8）・全 130 見出しはすべて残存**していた。原典に**表は 1 つも存在せず、脚注も存在しない**（本文中の `[^N]` はすべて参考文献の参照番号で、末尾に 204 件の文献一覧として展開されている。既定どおり訳出しない）。**欠落は 1 件**——**§3.2.3「Hierarchical Memory and Orchestration」の 2 項目の箇条書きが丸ごと**（クリップでは「Two design tendencies dominate this space:」の直後がいきなり次の見出しになっている）。既知の itemize 脱落パターンで、原ページから復元して該当位置へ挿入し、本文中に「訳注: 復元」と明記した（内容は「時空間的な資源の分離」＝MemGPT / MemoryOS のホット・コールド階層と、「認知的機能による意味の分離」＝MemoryBank / MIRIX / MemOS / xMemory の機能別分離）。加えて**冒頭の著者・所属欄が LaTeX の `\author` マクロ崩れで断片列になっていた**ため、原ページから復元した。
- 翻訳の範囲: 本文 §1〜9 を全訳。**番号つき見出し 69／レベル 5 の小見出し 60 とも原典と完全一致**。参考文献一覧と謝辞は既定どおり除外。
- ユーザーの決定（AskUserQuestion）: **(1) §4「Externalized Expertise: Skills」は `agent-skills` を新設して受ける**（tool-use-and-function-calling への吸収でも harness-engineering への追記でもなく）。理由は、本論文が「ツールは*操作*、プロトコルは*どう記述され呼ばれるか*、スキルは*タスクのクラスがどう遂行されるべきか*」と 3 層をはっきり分けており、その 3 層目に対応するページが wiki に無かったこと。**(2) 理論的レンズ（Norman の認知的人工物・Kirsh の相補的戦略・Hutchins の分散認知）は `harness-engineering` の冒頭節に置く**（llm-agents ではなく）。
- メモ: **本論文は [[summaries/2026-code-as-agent-harness]] が `zhou2026externalization` として繰り返し引いていた原典そのもの**である。ほぼ同時期・別グループによる同主題のサーベイが 2 本揃ったことになり、**独立に「ハーネスが第一級の設計対象である」「現在のベンチマークはハーネス／外部化の寄与を過少に測る」という同じ結論へ到達している**点は分野の収束の証拠として記録に値する（ただし**両者とも提案した評価指標を操作化していない**）。切り分けは、**本論文からは「なぜ効くのか（認知的人工物）」と「スキルの体系」を、Code as Agent Harness からは「PEV ループ」と「共有ハーネス基盤の 4 水準」を採る**とした。もう 1 つ記録に値するのは、本論文が**独自の実験・数値をまったく持たない概念枠組み論文**であること——AHE（+5.6 / −2.3 pp）や Dive into Claude Code（1.6% / 98.4%）と読み方を変える必要がある。著者ら自身が §4.7 冒頭で理論の適用を「主として理論的であって直接に経験的ではない」と明記しており、証拠の厚みも節によって大きく違う（スキルの境界条件は SkillProbe・SkillsBench・公開エコシステムの脆弱性調査という直接の証拠を持つ一方、「文脈依存の劣化」は著者ら自ら証拠が限られると書いている）。
- **データギャップ**: 本論文が §8.2 で扱う**身体化された外部化（大脳–小脳の分離）** は、VLA・SayCan・Voyager といった原典が本 wiki に 1 件も無いため**本論文経由の又聞き**に留まる（[[summaries/2026-code-as-agent-harness]] の ingest 時に記録した同じギャップが解消していない）。また本論文が引く 2026 年のプレプリント（MemEvolve・MemVerse・MemRL・xMemory・MemOS・SkillProbe・SkillsBench・PolySkill・MemSkill・InfiAgent・ANP・A2UI・AG-UI・UCP・AP2 等）は大半が未取得で、これらの記述は要約の水準に留まる。**Voyager（スキルの Discovered 経路の代表例）は [[agent-skills]] の新設によって「参照されているが未取得」の優先度が上がった。**

## [2026-08-10] ingest | Kimi K3: Open Frontier Intelligence

- 取り込み: `raw/papers/Kimi K3_ Open Frontier Intelligence.md`（Kimi Team / Moonshot AI, arXiv:2607.24653, 2026。ケース A: ar5iv クリップ）
- 作成: [[translations/2026-kimi-k3]], [[summaries/2026-kimi-k3]], **[[concepts/agent-environments]]（新設）**, `raw/assets/2026-kimi-k3/`（benchmark0727.png ＋ fig2〜fig16 の 16 枚）
- 更新: [[concepts/transformer-architecture]], [[concepts/mixture-of-experts]], [[concepts/llm-inference-optimization]], [[concepts/reinforcement-learning-from-human-feedback]], [[concepts/test-time-compute]], [[concepts/agent-evaluation]], [[concepts/agent-safety-and-guardrails]], [[concepts/llm-red-teaming]], [[concepts/tool-use-and-function-calling]], [[concepts/harness-engineering]], [[overview]], [[index]]
- クリップの状態: **全 156 見出し・表 5 個（Table 1〜5）は残存**。図は 16 個で扱いが 3 種に割れた——**Figure 1 はラスタ PNG（benchmark0727.png）、Figure 2・3・4・5・12・14・16 はインライン SVG としてクリップに残存、Figure 6・7・8・9・10・11・13・15 は ar5iv が pgfplots を未レンダリングでクリップに画像なし**（キャプションのみ）。**全 16 図を arXiv PDF（arXiv:2607.24653, 47pp）から PyMuPDF のキャプション-bbox レンダリングで取得**し `raw/assets/2026-kimi-k3/` に保存（Figure 1 は benchmark0727.png を使用）。SVG 図もベクタなので PDF ラスタで統一し、クリップの SVG は保存していない。
- 本文欠落（確定・復元済み）: **§6.1.1 Benchmarks の "Coding" 箇条書きが丸ごと欠落**（Reasoning & Knowledge / Agentic / Vision の 3 箇条書きは残存）。既知の itemize 脱落パターンで、ar5iv 原ページから復元して該当位置へ挿入し翻訳中に「［訳注: 復元］」と明記した（DeepSWE, ProgramBench, Terminal-Bench 2.1, FrontierSWE, SWE-Marathon, PostTrainBench, MLS-Bench-Lite, SciCode）。
- 脚注 6 件はすべて URL のみ（HuggingFace リポジトリ・FLA PR・MoonEP・AgentENV・minitriton・nano-kpu の GitHub）で、本文の `<sup>N</sup>` マーカー位置に訳注として URL を併記。`[^N]` は参考文献番号で既定どおり訳出しない。
- ユーザーの決定（AskUserQuestion）: **(1) アーキ深度 = 厚め**——要約に加えて transformer-architecture（KDA の下限つき減衰・全ランクゲート・NoPE・Block AttnRes・SiTU-GLU・Per-Head Muon）、mixture-of-experts（Stable/Normalized LatentMoE・Quantile Balancing・MoonEP の完全均衡）、llm-inference-optimization（MXFP4 QAT・MTP→EAGLE-3 draft・KDA-aware prefix cache）を実質更新。**(2) `agent-environments` を新設**——K2 のツール合成・K2.5 の PARL・V4 の mixed-RL に続く 4 本目として、エージェントの RL 訓練環境／タスク合成を主題化し、K3 の white-box modular harness / KG タスク合成 / AET / モック / AgentENV を集約。
- メモ: 本 wiki の Moonshot/DeepSeek モデルレポートの系譜（K2→K2.5→V4→gpt2-to-kimi3）の直系。事後学習の系譜が「専門家 → 統合」で読める——K2 の joint RL → V4 の OPD（ロジット蒸留）→ **K3 の MOPD（領域 × 努力の 9 教師をトークンごと OPD で統合）**。分割の軸が「検証可能/不可能」→「領域」→「領域 × 努力」と細かくなり、統合が一貫してオンポリシー蒸留へ収束している。**最も概念的に新しいのは white-box RL 環境で、ハーネスを訓練時に撹乱する変数にする**——[[harness-engineering]] の「部品はモデルが単独でできないことへの仮定」を訓練側から裏返した設計。ただし K3 自身の評価はハーネス指定に強く依存しており（同モデルでも Kimi Code と Claude Code で数点動く）、「ハーネスに過適合しない訓練」と「ハーネス指定に依存する評価」が同じレポートに同居する緊張を [[agent-evaluation]]・[[harness-engineering]] に記録した。
- **データギャップ**: K3 が引く KDA/Kimi Linear のテクニカルレポート [^56]・AttnRes [^59]・LatentMoE [^31]・MoonViT-V2・EAGLE-3 [^70]・MXFP4 [^102]・Firecracker（AgentENV の基盤）は未取得で、アーキ記述は gpt2-to-kimi3 経由の又聞きを含む。agent-environments が引く 2026 年のエージェント環境系原典（AgentENV・MiniTriton 等の GitHub）も未取得。

## [2026-08-11] ingest | Kimi Linear: An Expressive, Efficient Attention Architecture

- 取り込み: `raw/papers/KIMI LINEAR- AN EXPRESSIVE, EFFICIENT ATTENTION ARCHITECTURE.pdf`（Kimi Team, arXiv:2510.26692v2, 2025-11-01。**ケース B: PDF**）
- 作成: [[translations/2025-kimi-linear]], [[summaries/2025-kimi-linear]], `raw/assets/2025-kimi-linear/`（fig1〜fig7 の 7 枚）
- 更新: [[concepts/transformer-architecture]], [[concepts/positional-encoding]], [[concepts/llm-inference-optimization]], [[summaries/2026-kimi-k3]], [[overview]], [[index]]
- 新概念ページは作らず（KDA/Kimi Linear は landmark 手法として transformer-architecture 内に、NoPE/位置符号化としての読みは positional-encoding 内に、効率は llm-inference-optimization 内に記述）。
- **位置づけ**: 本論文は K3（[[summaries/2026-kimi-k3]]）と gpt2-to-kimi3（[[summaries/2026-gpt2-to-kimi3]]）が**又聞きで書いていた KDA の一次資料** [^56]。K3 ingest 時のデータギャップ「KDA/Kimi Linear のテクニカルレポート [^56]」を解消した。KDA・DPLR 制約・3:1 ハイブリッド・効率の記述が一次資料に接地。
- **最大の新規性は [[positional-encoding]] への貢献**: 同ページは NoPE を「本 wiki に原典がない」と記録していた（§(c) の NoPE 項・末尾「根拠の所在」）。本論文が **KDA を「学習可能な乗法的位置符号化」と読み、RoPE の直交性制約を外した一般化と位置づけ、フル層を NoPE にする**理論を与えるので、新節 **(e) KDA — 位置符号化を「学習可能な遷移行列」に畳む** を追加し、NoPE を「原典が無い」→「一次資料がある」へ移した。
- クリップ状態（PDF ケース B）: 全 8 節（§1〜8＝Intro/Preliminary/KDA/Architecture/Experiments/Discussions/Related Works/Conclusion）＋付録 B（チャンクワイズ並列性の導出・命題 1/2）・C（擬似コード Listing 1）・D（5.7T 結果・Table 8/9）を全訳。付録 A（Contributions＝貢献者名一覧）は K2.5 の先例に倣い訳出せず訳注で存在のみ明記。
- 図の扱い: `pdftotext -layout` を本文底本に、**数式は複数行版面で pdftotext が激しく崩れるため PDF を Read で確認して LaTeX 再構成**（式番号は原典一致）。図 7 枚を PyMuPDF のキャプション-bbox レンダリングで取得（fig2/3/4/7 は初回 over/under-capture を再レンダリングで修正——header 混入・パネル切れ・Eq.9 混入を除去し Read で全確認）。Table 1〜9 を markdown 化。
- メモ: 主結果は 1.4T の**自己申告の管理下比較**（同一レシピ・同一パラメータの内製比較。ベース/SFT どまりで対フロンティアではない）。「full attention 超え」は transformer-architecture の既存注記（provider-reported 問題・[[agent-evaluation]]）と整合させて割り引いて記述。KDA のチャネル減衰の数値精度弱点（$1/\Gamma$ オーバーフロー）を K3 の下限つき減衰が継いで直した関係を両ページに明記。
- **データギャップ**: 本論文が引く GDN/Gated DeltaNet [111]・DeltaNet [84]・GLA [114]・Mamba2 [16]・Comba [40]・RWKV7 [71]・DPLR/S4 [30]・Moonlight [62] の各原典は未取得（Table 6/7 の系譜記述は本論文経由の又聞き）。線形アテンションの系譜（DeltaNet・GLA・Mamba2）は本 wiki に一次資料がなく、次に読む候補。
