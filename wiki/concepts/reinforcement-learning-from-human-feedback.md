---
type: concept
aliases: [RLHF, RLVR, GRPO, PPO, post-training, 事後訓練, verifiable rewards, 検証可能報酬, reward model, 報酬モデル, alignment, アラインメント]
...
- **RLHF**（Reinforcement Learning from Human Feedback）— **人間の選好**を学習した報酬モデルを使う。指示追従・有用性・無害性の整合（アラインメント）が主目的で、InstructGPT（2022）が確立した系譜。標準的な解説が [[summaries/2022-rlhf-illustrated]]（InstructGPT 論文そのものは未 ingest）。
...
| --- | --- | --- | --- |
| 2022 | 古典的 RLHF（報酬モデル＋PPO） | 「良さ」を測る損失関数が書けない問題を、人間の**比較**から学習した報酬モデルで代替した | [[summaries/2022-rlhf-illustrated]] |
...
1. **事前学習**: 通常の目的で訓練済みの LM から始める。任意で追加の微調整を挟んでよい（OpenAI は「望ましい」人間生成テキストで、Anthropic は **HHH 基準**（helpful, honest, and harmless, 有用・誠実・無害）に関する文脈手がかりで元モデルを蒸留した）が必須ではない。核心は「多様な指示にうまく応答するモデル」を用意すること。
2. **報酬モデルの訓練**: 人間の選好をスカラーの報酬に変換するモデルを作る。
3. **PPO による微調整**: 初期モデルの**コピー**を、報酬モデルのスコアを最大化するように強化学習で更新する。
...
もうひとつの経験則: 報酬モデルは生成モデルと**同程度の理解容量**を要するらしい（当時の実例は OpenAI が 175B の LM に 6B の RM、Anthropic が双方 10B〜52B、DeepMind が双方 70B）。判定側を安いモデルで済ませようとする現代の設計への警告としても読める。
...
> **陳腐化に関する注記**: [[summaries/2022-rlhf-illustrated]] は 2022 年 12 月の記事で、上記のうち**骨格（比較から報酬を作る／スカラー化／KL 型の乖離ペナルティ／reward hacking の構造／相対比較の優位）以外はほぼ入れ替わっている**。特に、当時の実装ライブラリ 3 本柱（TRL / TRLX / RL4LMs）は TRL への一本化が進み、「大規模選好データセットは hh-rlhf のみ」「パラメータを凍結するのが普通」といった記述は現在成り立たない。個別の対応関係は同要約の「この記事の賞味期限」節に表で整理してある。
...
- [[summaries/2022-rlhf-illustrated]] — 古典的 RLHF（報酬モデル＋PPO）の根拠原典
