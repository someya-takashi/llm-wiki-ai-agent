...
## なぜ外部化が効くのか — 認知的人工物という理論的レンズ

**本ページの残りはすべて「ハーネスは効く」を前提に、その*作り方*を扱う。ではなぜ効くのか。** [[summaries/2026-externalization]]（上海交通大学ほか, 2026）が、この問いに本 wiki で唯一の理論的な答えを与えている。依拠するのは 1980〜90 年代の認知科学——**Donald Norman の認知的人工物（cognitive artifacts）** である。

> **買い物リストは生物学的な記憶容量を拡張しない。それは困難な*想起*の問題を*再認*の問題へ変える。地図は単にナビゲーションを「より強く」するのではない。それは隠れた空間的関係を可視の構造へ変換する。**

**核心は「人工物の力は*能力の増幅*ではなく*問題の再構造化*にある」という一点である。** Norman 自身の定式化——**「認知的人工物は人間の能力を変えない。それはタスクを変える」**——が、そのままハーネスに当てはまる。

**この一文が実務で効くのは、バラバラに見える設計判断を 1 つの基準で評価できるようになるからである。** 同サーベイは、外部化の 3 次元をそれぞれ**表現の変換（representational transformation）** として整理する。

| 外部化 | 何の負荷を外部化するか | 表現の変換 | 素の LLM のミスマッチ |
|---|---|---|---|
| **記憶** → [[agent-memory]] | **時間**（継続性） | **想起 → 再認** | コンテキストウィンドウは有限で、セッションの記憶は弱いか存在しない |
| **スキル** → [[agent-skills]] | **手続き**（一貫性） | **生成 → 合成** | 長い多段の手続きが、一貫して実行されるのではなく毎回**再導出**される（＝**分散**の問題） |
| **プロトコル** → [[model-context-protocol]] | **相互作用**（構造） | **場当たり → 構造化された契約** | ツール・サービス・協働者との相互作用が、自由形式のプロンプティングでは脆い |

**そして同サーベイは「ハーネスは第 4 の外部化ではない」と明示する**——それは**他の 3 つがその内側で作動するランタイム環境**である。この整理は本ページの 7 コンポーネント分類と衝突しない。**7 分類が「編集できる部品は何か」を数え上げるのに対し、3 分類は「どの認知的負荷を移したか」で切っている。** 前者は実装のチェックリスト、後者は設計の判断基準として使える。

**判断基準としての使い方は単純である**——**新しいハーネスの部品を足すとき、「これはモデルが直面するタスクの*形式*を変えているか、それとも情報を足しているだけか」を問う。** 後者なら、それは外部化ではなく[コンテキストの負荷](#設計論点)を作っているだけかもしれない。同サーベイはこれを ***minimal sufficiency*（最小十分性）** と呼ぶ——「**そのモジュールは実際にモデルの認知的負荷を*減らす*のか、単に 1 つ*増やす*だけなのか**」。

### 認知環境としてのハーネス

同サーベイはさらに 2 つの枠組みを重ねる。**David Kirsh の相補的戦略（complementary strategies）** は、**エージェントが内部でより強く考えるのではなく、外部環境を再組織して認知的仕事の一部をそこへ退避させる**ことで性能を上げると論じる。ハーネスに当てはめると**分業の線が引ける**——

| 側 | 担うもの |
|---|---|
| **ハーネス** | 永続性・鮮度の管理・関連性のフィルタリング・形式の検証・ライフサイクルの制御 |
| **モデル** | 解釈・文脈的な判断・例外処理 |

**Edwin Hutchins の分散認知**が最後の一般化を与える——**作動している知能は、モデルのパラメータ・外部の記憶ストア・実行可能なスキル・プロトコルの定義・ツールの面・監視の系・それらを統べるランタイムの制約にわたって分散している。** 結論として同サーベイは、ハーネスを**インフラの層ではなく*認知環境*** と呼ぶ。

> **「ハーネスは、情報・ツール・権限・手続きが、*望ましい振る舞いが実行しやすく望ましくない振る舞いが生成しにくくなる*ように配置された認知的ニッチである。既定値・フック・ファイルの境界・スキルの呼び出しのパターン・レビューのゲートはすべて、もっともらしい行動の空間を狭める構造化された規則性として働く。」**

**留保も必要である。** これらは LLM エージェントのために開発された理論ではなく、**著者ら自身が適用を「主として理論的であって直接に経験的ではない」と明記している**。説明力はあるが、**そこから検証可能な予測が導けるかは示されていない**。本ページの他の節が扱う定量的な知見（層別の価値・非加法性・後退への盲目）とは、証拠の性格が違うものとして読むべきである。
...
- **ハーネスを「訓練時に撹乱する変数」にする。** 上の「剥がす」（load-bearing でない部品を外す）と対をなす逆向きの操作が、Kimi K3（[[summaries/2026-kimi-k3]]）の **white-box RL 環境**である——**ハーネス（ツールIF・システムプロンプト・コンテキスト管理・スキル・記憶・サブエージェント）を差し替え可能なモジュールとして表現し、RL 中にタスクグループごとに異なる構成を動的に撹乱する**ことで、単一ハーネス（Kimi Code / Claude Code / Codex / OpenClaw / Hermes）への過適合を最初から作らない。「部品はモデルが単独でできないことへの仮定である」を訓練側から言い換えると、「**その仮定に依存しないよう、訓練分布にハーネスの多様性を含める**」になる。詳細と系譜（K2 のツール合成・K2.5 の swarm 環境・V4 のスペシャリスト環境）は [[agent-environments]] へ。**ただし K3 自身の評価はハーネス指定に強く依存しており（同じモデルでも Kimi Code と Claude Code で数点動く）、「ハーネスに過適合しない訓練」と「ハーネス指定に依存する評価」が同居する**——ハーネスを撹乱して訓練しても、測るときにはハーネスを固定せざるをえない、という緊張が残る（→ [[agent-evaluation]]）。
...
- [[agent-skills]] — 7 コンポーネントの「スキル」層を独立させたページ。仕様・発見・段階的開示・バインディング・合成
- [[agent-memory]] — 単独で最大の利得（+5.6 pp）を持つ層
- [[tool-use-and-function-calling]] — ツール層の設計
- [[summaries/2026-externalization]] — **なぜ外部化が効くのかの理論**（Norman の認知的人工物・Kirsh の相補的戦略・Hutchins の分散認知）と、記憶／スキル／プロトコルの 3 分割の根拠原典
- [[summaries/2026-kimi-k3]] — ハーネスを**訓練時に撹乱する変数**にする white-box RL 環境の根拠原典
- [[agent-environments]] — ハーネス撹乱を含む RL 訓練環境の全体
