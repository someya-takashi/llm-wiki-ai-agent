...
1. **読解**：原典を読み、主要な主張・手法・結果・前提・限界を把握する。画像リンクが埋め込まれている場合は、画像 URL も保持し、必要に応じて画像も参照する（後述「画像の扱い」を参照）。読解と並行して、**クリップ不良の兆候を点検する**（後述「クリップ不良への対処」）。
...
10. **コミット**：検証が終わったら、**ユーザーに確認せずそのままコミットまで行う**（2026-07-28 のスキーマ更新でユーザーが常時実施を指示）。`git add -A` で raw 原典・画像・wiki 側の全変更をステージし、既存フォーマット（英語の要約本文＋ `Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>`）でコミットする。コミット後に `git status` でワーキングツリーがクリーンなことを確認し、完了報告にコミットハッシュを含める。
...
Obsidian Web Clipper のクリップには**不良が高頻度で混入する**。クリップ済み markdown を鵜呑みにせず、以下の兆候を読解時に必ず点検し、見つけたら柔軟に復元する。

### 典型的な不良パターン（実際に発生した例）

- **マクロ脱落**：LaTeX マクロ（`\model` 等で定義された手法名・モデル名）が丸ごと消え、文が不自然になる（例: "we apply our approach, named, to..."、キャプションの主語欠落）。タイトル自体が壊れることもある。
- **脚注の本文欠落**：本文中に `<sup>N</sup>` のマーカーだけが残り、脚注テキストが失われる。末尾の `[^N]` 群が参考文献なのか脚注なのかの判別にも注意。
- **図表の欠落・取り違え**：図が丸ごと落ちる、図の画像が別の表・図のキャプションと結びつく（例: Figure 2 の画像が「Table 1」のキャプション位置に置かれ、本物の Table 1 の数値表が欠落）。TikZ/pgfplots 等で描かれた図は **ar5iv 自体に画像が存在しない**こともある。
- **表の崩れ**：空のスペーサ列の混入、multicolumn の崩れ、HTML テーブルの平坦化。
- **細かい artifact**：語の連結（"SakanaFugu"）、脱字（"ahighly"）、キャプション内の参照番号化け。

### 対処の原則

1. **兆候の点検**：不自然に途切れる文・主語のない文、図表番号の飛び（Figure 1 の次が Figure 3）、本文の図表言及数と実際の埋め込み数の不一致、`<sup>N</sup>` マーカーの対応先の有無、を読解時に確認する。
2. **原ページと照合して復元する**：frontmatter の `source:` URL（ar5iv・原記事）を `curl -A "Mozilla/5.0"` で再取得し、欠落した本文・脚注・表・キャプションを抽出して確定する。**復元の根拠は必ず取得した原文に置く**（記憶や推測から本文を創作しない）。マクロ脱落した固有名の復元は文脈判断でよいが、機械置換はしない（ReAct-IM のように残っている複合語もある）。
3. **復元不能なものは無理に埋めない**：原ページからも取得できない要素（未レンダリングの図等）は、キャプション訳のみ残して「訳注: 原ページに画像なし」と明記する。
4. **記録する**：復元・正規化した内容は (a) 翻訳ファイル冒頭の訳注ブロックに要点を明記し、(b) log.md のメモに何をどう復元したか（取得失敗があればその URL も）を残す。
5. **raw/ は不変**：原典ファイル自体は修正しない。復元はすべて wiki 側（translations / summaries）で行う。
6. **判断に迷う不良**（表の列が崩れなのか原典由来なのか等）は、原ページ照合でも確定できなければユーザーに確認する。
...
- **PDF 論文**：ar5iv が HTML 変換に失敗する論文は PDF で保存（`raw/papers/`）。→ **ケース B**（`pdftotext` / PyMuPDF が使えればテキストも図も抽出する）
...
### ケース B: PDF 原典（`raw/papers/`）— 抽出可能なら抽出する

**（2026-08-02 更新）** 以前この節は「PDF からは画像をプログラム的に取り出せない」ことを前提に画像なしを既定としていたが、**その前提は成り立たない**。環境に `pdftotext` と PyMuPDF（`import fitz`）があり、本文テキストも図もプログラム的に取得できる。**まず利用可否を確認し、使えるなら抽出する。**

**テキスト**：`pdftotext`（表の再構成には `-layout`、ページ範囲は `-f` / `-l`）または PyMuPDF で抽出する。**ページ画像を Read で読むより桁違いに安く正確**なので、翻訳の底本にはこちらを使う。ただし**構成の把握と図の内容確認には Read でページを開く**のが有効で、両者は併用する。

**図**：論文の図は多くが**ベクタ描画**なので、`page.get_images()` での埋め込み画像の抽出では取れない（取れるのはアイコン等だけ）。代わりに**キャプションの位置を境界として領域をレンダリング**する:

1. 各ページのテキストブロックから `Figure N:` / `Table N:` で始まるブロックの bbox を集める。
2. 図の下端＝キャプションの `bbox[1]`、上端＝その上にある**本文段落**（長いテキストブロック）の `bbox[3]`。
3. `page.get_pixmap(dpi=200, clip=fitz.Rect(...))` で切り出して保存する。
4. **左右に並ぶ図**（同じ `bbox[1]` を持つ 2 つのキャプション）はキャプションの x 範囲で分割する。
5. 保存後は必ず **Read で開いて中身を確認**し、切れていたら範囲を調整する。

**何を画像にし、何をテキストに起こすか**：

- **画像にする**——ダイアグラム・プロット・ヒートマップ・スクリーンショット・並列比較図。
- **テキストに起こす**——**中身が文字だけの「箱」**（プロンプト・設定ファイル・コード・ログの囲み）。これらは画像化しても検索・引用できないので、抽出したテキストを本文へ収録する。ケース A/C の SVG テキスト箱と同じ判断。
...
**抽出ツールが無い環境**では従来どおり画像なしで作成してよい（図に言及する場合はキャプションのテキスト訳のみ残す）。その旨を log.md に残す。
